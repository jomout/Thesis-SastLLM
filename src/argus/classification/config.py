...
from argus.ml.models.config import MLPModelConfig, RepositoryModelConfig, parse_model_config
...
class ClassificationConfig:
    ...
    def from_yaml(cls, mode: ClassificationMode, config_path: str | Path = "configs/classification.yaml") -> ClassificationConfig:
        ...
        if not isinstance(raw, dict):
            raise TypeError(f"Missing 'classification.{mode}' section in {config_path}.")

        params = raw.get("params", {})
        if not isinstance(params, dict):
            raise TypeError(f"'classification.{mode}.params' must be a mapping.")
        ...
        if not isinstance(models, dict):
            raise TypeError(f"Missing top-level 'models' mapping in {config_path}.")
...
def _parse_grid_search(raw: Any) -> dict[str, tuple[Any, ...]]:
    if raw is None:
        return {}
    if not isinstance(raw, dict):
        raise TypeError("grid_search must be a mapping of parameter name to list of values.")
    grid: dict[str, tuple[Any, ...]] = {}
    for key, values in raw.items():
        if not isinstance(key, str):
            raise TypeError("grid_search parameter names must be strings.")
