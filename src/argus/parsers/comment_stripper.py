...
class CommentStripper:
    ...
    def strip(
        ...
        *,
        ...
        """
        if not language:
            logger.exception("Comment stripping requires an explicit language", filename=filename)
    ...
    def _get_parser(self, language: str) -> Parser:
        ...
        try:
            ...
            logger.exception("Unable to create comment parser", language=language, error=str(e))
    ...
    @staticmethod
    def _byte_index_after_n_lines(code: str, n_lines: int) -> int | None:
        if n_lines <= 0:
            return None
        idx = 0
        count = 0
