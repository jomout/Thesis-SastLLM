...
from argus.configs import get_logger, settings
...
class EmbeddingsManager:
    ...
    def insert_embeddings(
        ...
        if not embeddings:
            logger.exception("Cannot insert an empty embedding batch", collection=collection_name)
    ...
    def update_payload_by_filter(
        ...
        def build_filter(f: dict) -> models.Filter:
            ...
            for key, value in f.items():
                if isinstance(value, dict):
                    ...
                    raise TypeError(f"Unsupported filter value type for key '{key}': {type(value)}")
        ...
        try:
            ...
            logger.exception("Qdrant payload update failed", collection=collection_name, error=str(e))
