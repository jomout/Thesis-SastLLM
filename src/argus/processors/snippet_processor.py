...
from argus.analyzers import FunctionalityAnalyzer
from argus.configs import get_logger
from argus.db import FunctionalityManager, SnippetManager
from argus.dtos import CreateFunctionalityDto, UpdateSnippetDto
from argus.entities import SnippetWithFileAndRepository
from argus.parsers import TreeSitterGenerator
from argus.prompt import FunctionalityPromptGenerator
from argus.utils import Normalizer
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        if logger:
                            logger.exception(f"{func.__name__} failed after {max_retries} attempts")
                        raise
                    if logger:
...
class SnippetProcessor:
    ...
    def run(self) -> None:
        ...
        for i, batch in tqdm(enumerate(snippet_batches), desc="Analyzing snippets", total=total_batches):
            ...
            try:
                ...
                break
            except Exception as e:  # noqa: BLE001 -- terminate the pipeline cleanly on an unexpected batch failure
    ...
    def _cache(dir: str, snippet_id: int, payload: list[CreateFunctionalityDto]) -> None:
        ...
        except Exception as file_err:  # noqa: BLE001 -- caching is best-effort and must not fail processing
    ...
    def _store_functionalities(self, extracted_data: dict[int, list[str]]) -> None:
        ...
        for key, descriptions in extracted_data.items():
            try:
                ...
                try:
                ...
                except Exception as e:  # noqa: BLE001 -- bulk failure deliberately falls back to individual inserts
                    ...
                    for item in payload:
                        ...
                        except Exception as e2:  # noqa: BLE001 -- continue after one failed fallback insert
                            logger.warning(f"Failed to add functionality for {snippet_id}: {e2}")
                            continue
            except Exception as e:  # noqa: BLE001 -- malformed model output is skipped at this boundary
